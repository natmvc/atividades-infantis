      ...
      <section className="px-4 py-14 sm:px-6">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <Reveal className="overflow-hidden rounded-[2.5rem] bg-white p-3 shadow-soft ring-1 ring-sky-100">
            <Image src={theme.image} alt={`${product.title} ${theme[locale].name}`} width={1400} height={1050} className="aspect-[4/3] rounded-[2rem] object-cover" priority />
          </Reveal>
          <Reveal delay={0.1}>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {["Capa", "Mockup", "Galeria"].map((label) => (
              <div key={label} className="relative h-64 overflow-hidden rounded-[2rem] bg-skywash shadow-soft ring-1 ring-sky-100">
                <Image src={theme.image} alt={`${label} ${product.title}`} fill className="object-cover" />
              </div>
            ))}
          </div>
        </div>
